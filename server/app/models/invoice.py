...
class Invoice(Base):
    ...
    discount_requested_amount = Column(Numeric(12, 2), nullable=False, default=0, server_default='0')
    discount_status = Column(String(20), nullable=False, default='none', server_default='none')
    discount_reviewed_at = Column(DateTime(timezone=True), nullable=True)
    discount_reviewed_by = Column(Integer, ForeignKey('users.id', ondelete='SET NULL'), nullable=True)
    discount_reject_reason = Column(Text, nullable=True)
    ...
    is_outside_operating_hours = Column(Boolean, nullable=False, default=False, server_default='false')
    ...
    discount_reviewer = relationship('User', foreign_keys=[discount_reviewed_by])
