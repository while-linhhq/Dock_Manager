...
export type BackupAuditLogListProps = {
  ...
  filtered: AuditLogRead[];
  onRefresh: () => void;
};

export const BackupAuditLogList: React.FC<BackupAuditLogListProps> = ({
  isLoading,
  auditLogs,
  filtered,
  onRefresh,
  ...
  return (
    <div className="bg-white dark:bg-[#121214] border border-gray-200 dark:border-white/5 rounded-2xl shadow-2xl overflow-hidden">
      <div className="p-6 border-b border-gray-200 dark:border-white/5 flex justify-between items-center flex-wrap gap-2">
      ...
      <div className="divide-y divide-gray-100 dark:divide-white/5">
        {isLoading && auditLogs.length === 0 ? (
          ...
          filtered.map((log) => (
            <div
              key={log.id}
              className="p-6 hover:bg-gray-50 dark:hover:bg-white/[0.01] transition-colors group"
            >
          ...
          <div className="p-12 text-center text-gray-500 text-xs uppercase font-mono tracking-widest">
            {auditLogs.length === 0
